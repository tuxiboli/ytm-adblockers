...
import { installAdBlocker } from "./ad-blocker";
...
log.errorHandler.startCatching({
  ...
  onError({ error, processType, versions }) {
    ...
    if (!app.isReady()) {
      dialog.showErrorBox(`YTM Adblockers Crashed`, `Application crashed before ready\n\n${dialogMessage}`);
    } else {
      ...
      result = dialog.showMessageBoxSync({
        title: "Error",
        message: "YTM Adblockers Crashed",
      ...
      if (result === 0 || result === 2) {
        clipboard.writeText(`YTM Adblockers Crashed\n\n${dialogMessage}`);
      }
    }
...
const template: MenuItemConstructorOptions[] = [{ role: "appMenu", label: "YTM Adblockers" }, { role: "editMenu" }];
...
// Performans: Windows'ta pencere gizlendiğinde/kapatıldığında Chromium'un
// "pencere görünmüyor" tespiti oynatmayı duraklatabiliyor/donuklaştırabiliyor.
// Mini player modunda ana pencere gizli çalıştığı için bunu devre dışı bırakıyoruz.
...
function setupTaskbarFeatures() {
  ...
  // PERF: Değişiklik takibi — sadece değer GERÇEKTEN değişince native çağrı / IPC yapılır.
  // Eski kod her stateChanged'da (şarkı çalarken saniyede ~4 kez!) thumbbar butonlarını
  // yeniden oluşturuyor (3 PNG diskten decode) ve mini player'a tüm kuyruğu
  // yeniden map'leyip 4 IPC mesajı gönderiyordu. Bu sürekli CPU/IPC yükü demekti.
  type ThumbbarIcons = {
    previous: ReturnType<typeof nativeImage.createFromPath>;
    play: ReturnType<typeof nativeImage.createFromPath>;
    pause: ReturnType<typeof nativeImage.createFromPath>;
    next: ReturnType<typeof nativeImage.createFromPath>;
  };
  let thumbbarIcons: ThumbbarIcons | null = null;
  const getThumbbarIcons = (): ThumbbarIcons => {
    if (!thumbbarIcons) {
      thumbbarIcons = {
    ...
    return thumbbarIcons;
  };

  let lastThumbHasVideo: boolean | null = null;
  let lastThumbIsPlaying: boolean | null = null;
  let lastProgressUpdate = 0;
  let lastMiniProgressSent = 0;
  let lastMiniIsPlaying: boolean | null = null;
  let lastMiniIndex: number | null = null;
  let lastMiniVideoId: string | null = null;
  let lastMiniLike: number | null = null;
  let lastMiniRepeat: string | null = null;
  let lastMiniQueueKey = "";

  playerStateStore.addEventListener((state: PlayerState) => {
    const hasVideo = !!state.videoDetails;
    const isPlaying = state.trackState === VideoState.Playing || state.trackState === VideoState.Buffering;
    const now = Date.now();

    if (process.platform === "win32" && mainWindow && mainWindow.isVisible()) {
      // Thumbbar'ı sadece parça/oynatma durumu değişince güncelle (ilerleme tick'lerinde değil)
      if (hasVideo !== lastThumbHasVideo || isPlaying !== lastThumbIsPlaying) {
        lastThumbHasVideo = hasVideo;
        lastThumbIsPlaying = isPlaying;
        const icons = getThumbbarIcons();
        const taskbarFlags = [];
        if (!hasVideo) {
    ...
    // İlerleme çubuğu: her tick yerine en fazla saniyede 1 güncelle
    if (mainWindow && store.get("playback.progressInTaskbar") && now - lastProgressUpdate >= 1000) {
    ...
    if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
      ...
      // playerState: oynatma durumu/parça indeksi değişince anında,
      // progress değeri ise en fazla saniyede 1 gönderilir
      const progressDue = now - lastMiniProgressSent >= 1000;
      if (progressDue || isPlaying !== lastMiniIsPlaying || currentIndex !== lastMiniIndex) {
        lastMiniIsPlaying = isPlaying;
        lastMiniIndex = currentIndex;
        if (progressDue) lastMiniProgressSent = now;
        miniPlayerWindow.webContents.send("miniPlayer:playerState", {
          isPlaying: isPlaying,
      ...
      if (hasVideo) {
        const likeStatus = state.videoDetails.likeStatus;
        // trackInfo: sadece parça veya like durumu değişince gönderilir
        if (state.videoDetails.id !== lastMiniVideoId || likeStatus !== lastMiniLike) {
          lastMiniVideoId = state.videoDetails.id;
          lastMiniLike = likeStatus;
          const thumbnail = state.videoDetails?.thumbnails?.[0]?.url || "";
          const likeText = likeStatus === 2 ? "LIKE" : likeStatus === 0 ? "DISLIKE" : "INDIFFERENT";
          miniPlayerWindow.webContents.send("miniPlayer:trackInfo", {
            title: state.videoDetails?.title || "YouTube Music",
      ...
      if (state.queue) {
        const repeatMode = state.queue.repeatMode === 1 ? "ALL" : state.queue.repeatMode === 2 ? "ONE" : "NONE";
        if (repeatMode !== lastMiniRepeat) {
          lastMiniRepeat = repeatMode;
          miniPlayerWindow.webContents.send("miniPlayer:repeatChange", repeatMode);
        }

        // queueUpdate: sadece kuyruk gerçekten değişince map edilir ve gönderilir.
        // (Kimlik kontrolü ucuz: uzunluk + seçili indeks + ilk/son videoId)
        const items = state.queue.items;
        const firstId = items.length > 0 ? items[0].videoId : "";
        const lastId = items.length > 0 ? items[items.length - 1].videoId : "";
        const queueKey = `${items.length}:${state.queue.selectedItemIndex}:${firstId}:${lastId}`;
        if (queueKey !== lastMiniQueueKey) {
          lastMiniQueueKey = queueKey;
          ...
          miniPlayerWindow.webContents.send("miniPlayer:queueUpdate", queueItems);
        }
      }
    }
  });
...
// Pencereyi güvenilir şekilde görünür yapar.
// Windows'ta minimize edilmiş haldeyken gizlenmiş pencereye sadece show()
// uygulamak pencereyi görünmez/ilgisiz durumda bırakabiliyor (tray Show/Hide'da
// "ilk basışta açılmıyor" bug'ının sebebi). restore() + show() + focus() şart.
...
// Ana pencereyi açar ve mini player açıkca kapatır (tek yerden yönetilir)
function showMainWindowAndCloseMiniPlayer() {
  showMainWindow();
  if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
...
const createYTMView = (): void => {
  ...
  ytmView = new BrowserView({
    webPreferences: {
      ...
      // Mini player / gizli pencere modunda müziğin durmaması/teklememesi için şart
      backgroundThrottling: false,
      // Sözlük indirmeyi ve gereksiz DOM çalışmasını engeller
      spellcheck: false
    }
  });
  installAdBlocker(ytmView.webContents.session, ytmView.webContents);
  companionServer.provide(store, memoryStore, ytmView);
  customCss.provide(store, ytmView);
  ratioVolume.provide(ytmView);
  ...
  ytmView.webContents.on("page-title-updated", (_event, title) => {
    if (mainWindow) {
      mainWindow.setTitle(`${title} | YTM Adblockers`);
    }
  });
...
const createMainWindow = (): void => {
  ...
  mainWindow.on("close", event => {
    if (!applicationQuitting) {
      event.preventDefault();
      mainWindow.hide();
      // Ana pencere ve mini player ikisi de yoksa uygulamayı kapat
      if (miniPlayerWindow === null || miniPlayerWindow.isDestroyed()) {
        app.quit();
      }
    }
...
const createMiniPlayerWindow = (): void => {
  ...
  if (savedBounds) {
    ...
    posX = screenWidth - miniWidth - 16;
    posY = screenHeight - miniHeight - 16;
  }
  ...
  miniPlayerWindow.on("move", () => {
    if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
      ...
      store.set("state.miniPlayerBounds", { x: bounds.x, y: bounds.y, width: bounds.width, height: bounds.height });
    }
  });
...
app.on("ready", async () => {
  ...
  try {
    ...
    try {
      ...
      const migrateDialog = await dialog.showMessageBox({
        ...
        detail:
          "A configuration file for YTM Adblockers v1 was found. Your settings can be migrated.\n\nWARNING: Not all settings will be migrated as they may no longer be available in v2.",
  ...
  // Handle mini player ipc
  ipcMain.on("miniPlayer:open", () => {
    if (miniPlayerWindow !== null && !miniPlayerWindow.isDestroyed()) {
      miniPlayerWindow.close();
    } else {
      createMiniPlayerWindow();
      // Mini player açılınca ana pencereyi gizle
  ...
  ipcMain.on("miniPlayer:playPause", () => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "playPause");
    }
  });

  ipcMain.on("miniPlayer:next", () => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "next");
    }
  });

  ipcMain.on("miniPlayer:previous", () => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "previous");
    }
  });

  ipcMain.on("miniPlayer:setVolume", (_event, volume: number) => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "setVolume", volume);
    }
  });

  ipcMain.on("miniPlayer:toggleRepeat", () => {
    if (ytmView) {
      const state = playerStateStore.getState();
      let currentRepeat = "NONE";
      if (state.queue) {
        currentRepeat = state.queue.repeatMode === 1 ? "ALL" : state.queue.repeatMode === 2 ? "ONE" : "NONE";
      }
      let newRepeat = "NONE";
      if (currentRepeat === "NONE") newRepeat = "ALL";
      else if (currentRepeat === "ALL") newRepeat = "ONE";
      else newRepeat = "NONE";
      ytmView.webContents.send("remoteControl:execute", "repeatMode", newRepeat);
      if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
        miniPlayerWindow.webContents.send("miniPlayer:repeatChange", newRepeat);
      }
    }
  });

  ipcMain.on("miniPlayer:toggleLike", () => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "toggleLike");
    }
  });

  ipcMain.on("miniPlayer:toggleDislike", () => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "toggleDislike");
    }
  });

  ipcMain.on("miniPlayer:seekTo", (_event, time: number) => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "seekTo", time);
    }
  });

  ipcMain.on("miniPlayer:openMain", () => {
    if (mainWindow) {
      showMainWindowAndCloseMiniPlayer();
    } else if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
      miniPlayerWindow.close();
    }
  });
  ...
  ipcMain.on("miniPlayer:close", () => {
    ...
    // Mini player ve ana pencere ikisi de yoksa uygulamayı kapat
    if (mainWindow === null || mainWindow.isDestroyed() || !mainWindow.isVisible()) {
      app.quit();
    }
  });

  ipcMain.on("miniPlayer:playQueueIndex", (_event, index: number) => {
    if (ytmView) {
      ytmView.webContents.send("remoteControl:execute", "playQueueIndex", index.toString());
    }
  });

  ipcMain.on("miniPlayer:requestInitialData", () => {
    if (miniPlayerWindow === null) return;

    const state = playerStateStore.getState();
    const thumbnail = state.videoDetails?.thumbnails?.[0]?.url || "";
    const likeStatus = state.videoDetails?.likeStatus === 2 ? "LIKE" : state.videoDetails?.likeStatus === 0 ? "DISLIKE" : "INDIFFERENT";
    miniPlayerWindow.webContents.send("miniPlayer:trackInfo", {
      title: state.videoDetails?.title || "YouTube Music",
      author: state.videoDetails?.author || "",
      thumbnail: thumbnail,
    ...
    miniPlayerWindow.webContents.send("miniPlayer:playerState", {
      isPlaying: state.trackState === 1 || state.trackState === 2,
      progress: state.videoProgress || 0,
      currentIndex: state.queue?.selectedItemIndex ?? 0
    });
    miniPlayerWindow.webContents.send("miniPlayer:volumeChange", state.volume || 100);

    let repeatMode = "NONE";
    if (state.queue) {
      repeatMode = state.queue.repeatMode === 1 ? "ALL" : state.queue.repeatMode === 2 ? "ONE" : "NONE";
    }
    miniPlayerWindow.webContents.send("miniPlayer:repeatChange", repeatMode);

    if (state.queue) {
      const queueItems = state.queue.items.map((item, index) => ({
        title: item.title,
        artist: item.author,
        thumbnail: item.thumbnails?.[0]?.url || "",
        index: index
  ...
  trayContextMenu = Menu.buildFromTemplate([
    ...
    },
    {
      ...
      click: () => {
        if (mainWindow) {
          ...
          const miniExists = miniPlayerWindow && !miniPlayerWindow.isDestroyed();
          const miniVisible = miniExists && miniPlayerWindow.isVisible();

          if (mainVisible || miniVisible) {
            ...
            // Gizli mini player penceresi var -> mini player moduna geri dön
            miniPlayerWindow.show();
            miniPlayerWindow.focus();
          } else {
            showMainWindowAndCloseMiniPlayer();
    ...
    },
    {
      ...
      click: () => {
        if (miniPlayerWindow && !miniPlayerWindow.isDestroyed()) {
          // Mini player açik -> kapat, ana pencereyi geri getir
          miniPlayerWindow.close();
          showMainWindow();
        } else {
          // Mini player aç, ana pencereyi gizle
          createMiniPlayerWindow();
  ...
  tray.setToolTip("YTM Adblockers");
